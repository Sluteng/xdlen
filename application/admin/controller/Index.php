...
class Index extends Controller
{
    ...
    public function welcome()
    {
        ...
        //查询后台登录次数
         $numResult=Db::name('login_num')->where("uid", UID)->order('login_time desc')->limit(7)->select();
         $arrs=i_array_column($numResult,'num');
         $nums = implode(",", $arrs); //数组转换字符串 return json($info);
        //查询后台浏览次数
        $numberResult=Db::name('number')->where("uid", UID)->order('create_at desc')->limit(7)->select();
        $arr=i_array_column($numberResult,'create_at');
        $times = "'" . implode("','", $arr) . "'"; //数组转换带单引号字符串
        $arr1=i_array_column($numberResult,'time');
        $titles = implode(",", $arr1);  //数组转换字符串
        return $this->view->fetch('',[
        'nums'=>$nums,
        'times'=>$times,
        'titles'=>$titles,
